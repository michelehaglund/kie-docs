...
The following sections describe each asset in {PRODUCT} {PRODUCT_VERSION}.
...
* DSL definition
+
Domain Specific Languages (DSLs) are rule languages that are dedicated to problem domains.
...
* Package
+
All assets are contained in packages in Drools Workbench. A package is like a folder (it also serves as a "namespace"). A home folder for rule assets to live in.

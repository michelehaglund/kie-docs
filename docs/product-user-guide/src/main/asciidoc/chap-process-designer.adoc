...
. Set the marshalling strategy in the `_PROJECT_HOME_/META_INF/kie-deployment-descriptor.xml`. You can do so in the Business Central or by editing the file directly. In Business Central:

.. In Business Central, go to *Menu -> Design -> Projects* and click on the project name. For example, *Evaluation*.
.. Click *Open Project Editor*. Then, click *Project Settings: Project General Settings* -> *Deployment descriptor*.
...
----
+
After you correctly set the marshalling strategy, create a document process variable. This step is required for the document to be visible in the *Documents* tab of the *Process Management* -> *Process Instances* view in Business Central.

. In Business Central, navigate to your business proces and open it in the Business Process Designer.
...
This constraint condition retrieves the process variable [var]``name``.
====
...
. Click *Authoring* -> *Artifact repository*. *<MH> Dev reviewers, is this still possible, or should we remove it?</MH>*
...
Technical components used in a process fail in a way that cannot be described using BPMN (for further information, see _{DEVELOPMENT_GUIDE}_).

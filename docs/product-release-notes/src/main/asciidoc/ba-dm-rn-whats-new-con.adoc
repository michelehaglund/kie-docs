...
*<MH>SME reviewer* - I have added most of what I can find from the https://docs.google.com/document/d/1vypMG1ple4OXG9krcZ5-VbsQm_AGCOap_7pe6pMI4qs/edit#heading=h.nb7wjb26zsjt doc. Please advise on anything that needs correction. Also, let me know if I should remove anything that definitely does pertain to RHDM.</MH>

.Renamed components
--
[cols="1,2,3",grid="all",options="header"]
|===
| Component
| Old name
| New name
...
| Planner *<MH>Need SME clarification of the following for "Planner" and "Business Resource Planner". Which of the two rows is best?
| Business Resource Planner
| Business Optimizer

| Business Resource Planner
| OptaPlanner; planner
| Resource Optimizer, OptaPlanner, or just Planner
...
| Process server
| Process server; KIE server
| Process Server

| Decision server
| Decision server; KIE server
| Decision Server
...
| Decision engine, Process engine

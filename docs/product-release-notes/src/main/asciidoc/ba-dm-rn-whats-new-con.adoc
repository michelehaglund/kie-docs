...
| Decision Server (for Decision Management capabilities)

| KIE Execution Server
| Decision Server, Planner Server, KIE Server (represents both Decision and Planner servers )
...
=== Packages
A package includes a collection of application templates. You can download any available package containing the application that meets your business requirements. {PRODUCT} {PRODUCT_VERSION} will contain the *Employee Roster* template *<MH> or Package</MH>*. The *Vehicle Routing* template *<MH> or Package</MH>* will be added in {PRODUCT} 7.1 and Red Hat Business Automation 7.0.

The notion here is over time we continue to build this package with additional templates that helps drive adoption of our technologies.

=== Package delivery
This package would be downloadable from Customer Portal.
Included in both DM and BA distributions.
This package delivery could coincide with the product GA release or a follow on distributions.
In theory we should be able to update the content of this package anytime.

=== Package contents
For immediate release, it would only include Employee Roster template.  Fro DM 7.1 / BA 7.0 release it would include Vehicle Routing as well.
We will continuously evaluate possible value adds that we could include in the package.

=== Package support
This package is not an integral part of the product.  It is a template that we encourage customers to adopt, modify and make it work for their needs.
This is intended for development purposes of customer solution.
The content of the package itself does not carry specific product SLAs.  We encourage customers to report issues, feedback and request for enhancements.
We would consider them for future updates of the templates.
However the standard support entitlement of bug fixes, patches etc is limited to product bits only.

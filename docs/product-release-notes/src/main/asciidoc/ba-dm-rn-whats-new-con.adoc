...
=== Package support
Packages are not an integral part of {PRODUCT} and Red Hat Business Automation distributions and are intended for development purposes of customer solutions. They are provided to encourage customers to adopt and modify them to work for their specific needs.
The content of the package itself does not carry specific product Service Level Agreements (SLAs). We encourage you to report issues, request for enhancements, and any other feedback, which we will consider for package updates.

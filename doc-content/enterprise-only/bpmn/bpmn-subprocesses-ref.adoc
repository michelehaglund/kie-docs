...
* *Abort Parent*: If selected, non-independent, reusable subprocesses can abort the parent process when there's an error during the execution of the called process instance. For example, when there's an error when trying to invoke the subprocess or when the subprocess instance is aborted. This property is only visible when the *Independent* property is not selected. The following rules apply:
...
* *Wait for completion* : If selected, the subprocess will continue if the previous process has completed or aborted its execution; otherwise it will continue immediately after starting the subprocess and not wait for the process to complete. This property is selected by default.

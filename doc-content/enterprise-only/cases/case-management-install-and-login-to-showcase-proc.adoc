...
ifdef::PAM[]
`_TEMP_DIR/rhpam-{PRODUCT_VERSION}-case-mgmt-showcase-eap7-deployable/jboss-eap-{PRODUCT_VERSION}`
endif::PAM[]
ifdef::DM[]
`_TEMP_DIR/rhdm-{PRODUCT_VERSION}-case-mgmt-showcase-eap7-deployable/jboss-eap-{PRODUCT_VERSION}`
endif::DM[]
...
. Add the following system property to your deployment’s `{EAP_VERSION}/jboss-eap-7.3/standalone/configuration/standalone-full.xml` file:
+

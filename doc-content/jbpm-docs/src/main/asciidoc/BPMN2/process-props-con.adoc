...
* Does the implementation define any other actions that need to be taken when an exception is caught? Would it be beneficial to interact with other technical systems? Should a subprocess be triggered to handle the exception?
...
The following example uses a Throwing Error Intermediate Event to throw an error. An Error Event subprocess then catches and handles the error.
...
.Process with an exception handling Event subprocess

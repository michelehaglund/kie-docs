...
.Types of Intermediate Events in Object Library
[cols="25%,10%,65%a", frame="all", options="header"]
|===
|Event
|Icon
...
|*Intermediate Signal*
|image:processes/6670.png[]
|Use the `Intermediate Signal` throwing intermediate event to produces a signal object. Once it creates a signal object, the process execution continues. The signal object is consumed by a `Signal` start event or a Signal catching intermediate event, which is looking for this specific signal object.

|*Intermediate Compensation*
|image:processes/6671.png[]
|Use the `Intermediate Compensation` throwing intermediate event to handle compensation in case of partially failed operations.

|*Intermediate Escalation*
|image:processes/6672.png[]
|Use the `Intermediate Escalation` throwing event to produce an escalation object. Once it creates an escalation object, the process execution continues. The escalation object can be consumed by an `Intermediate Escalation` start event or an `Intermediate Escalation` catch event, which is looking for this specific escalation object.

|*Intermediate Message*
|image:processes/6673.png[]
|Use the `Intermediate Message` throwing intermediate event to produce and send a message to a communication partner (such as an element in another process). Once it sends a message, the process execution continues.
|===
...
|*Intermediate Timer*
|image:processes/6680.png[]
|Use the `Intermediate Timer` intermediate event to delay the workflow execution until a specified point or duration. A `Intermediate Timer` intermediate event has one incoming flow and one outgoing flow and its execution starts when the incoming flow transfers to the event. When placed on an activity boundary, the execution is triggered at the same time as the activity execution.

|*Intermediate Error*
|image:processes/6681.png[]
...
|image:processes/6682.png[]
|Use the `Intermediate Conditional` intermediate event to execute a workflow when a specific business Boolean condition that it defines, evaluates to true. When placed in the process workflow, a `Intermediate Conditional` intermediate event has one incoming flow and one outgoing flow and its execution starts when the incoming flow transfers to the event. When placed on an activity boundary, the execution is triggered at the same time as the activity execution. Note that if the event is non-interrupting, it triggers continuously while the condition is true.

|*Intermediate Compensation*
|image:processes/6683.png[]
|Use the `Intermediate Compensation` intermediate event to handle compensation in case of partially failed operations. A `Intermediate Compensation` intermediate event is a boundary event that is attached to an activity in a transaction subprocess that may finish with a `Intermediate Compensation` end event or a `Cancel` end event. The `Intermediate Compensation` intermediate event must have one outgoing flow that connects to an activity that defines the compensation action needed to compensate for the action performed by the activity.

|*Intermediate Signal*
|image:processes/6684.png[]
|Use the `Intermediate Signal` catching intermediate event to execute a workflow once a specified signal object defined in its properties is received from the main process or any other process.

|*Intermediate Escalation*
|image:processes/6685.png[]
|Use the `Intermediate Escalation` catching intermediate event in your process to consume an Escalation object. An `Intermediate Escalation` catching intermediate event awaits a specific escalation object defined in its properties. Once it receives the object, it triggers execution of its outgoing flow.

|*Intermediate Message*
|image:processes/6686.png[]
|Use the `Intermediate Message` catching intermediate event in your process to implement a reaction to an arriving message. The message that this event is expected to react to, is specified in its properties. It executes the flow only when it receives the specific message.
|===
...
|Use the `Multiple Instance` subprocess when you want to execute the contained subprocess elements multiple number of times.
...
A `Multiple Instance` subprocess is completed when the condition specified in the `MI completion condition` property is satisfied.
|===
...
.Types of Gateway in Object Library
[cols="1,1,3a", frame="all", options="header"]
|===
|Subprocess
|Icon
...
|*Parallel* (AND)
|image:processes/6690.png[]
|In a converging gateway, waits for all incoming flows. In a diverging gateway, takes all outgoing flows simultaneously.

|*Event*
|image:processes/6691.png[]
|used only in diverging gateways for reacting to events. See <<_event_based_gateway>>.

|*Exclusive* (XOR)
|image:processes/6692.png[]
|In a converging gateway, only the first incoming flow whose condition evaluates to true is chosen. In a diverging gateway only one outgoing flow is chosen.

|*Inclusive* (OR)
|image:processes/6693.png[]
|In a converging gateway, waits for all incoming flows whose condition evaluates to true. In a diverging gateway takes all outgoing flows whose condition evaluates to `true`.
|===
